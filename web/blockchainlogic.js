...
async function bridgingEth(_jobId, _newFromToken) {
  try {
    ...
    const maticPos = new Matic.MaticPOSClient({
      ...
      maticProvider:
        "https://speedy-nodes-nyc.moralis.io/cff6f789838e10c4008b1baa/polygon/mainnet", //"https://speedy-nodes-nyc.moralis.io/cff6f789838e10c4008b1baa/polygon/mumbai",
    });
    ...
    if (job.attributes.fromChain == 0 && job.attributes.toChain == 2) {
      ...
      if (
        ...
        txHash = await maticPos.depositEtherForUser(
          _userAddress,
      ...
      //Deposit any other PoS Token from Ether to Polygon
        ...
        await maticPos.approveERC20ForDeposit(
          job.attributes.fromTokenAddress,
          job.attributes.amount,
          { from: _userAddress }
        );
        txHash = await maticPos.depositERC20ForUser(
          job.attributes.fromTokenAddress,
          _userAddress,
          job.attributes.amount,
          { from: _userAddress }
      ...
      //Deposit any PosS Token from Polygon to Ether
      let burnTxHash = await maticPosEthBack.burnERC20(
        job.attributes.fromTokenAddress,
        job.attributes.amount,
        { from: _userAddress }
      );
...
async function bridgingMatic(_jobId, _newFromToken) {
  try {
    ...
    //Approve Polygon
    await maticPlasma.approveERC20TokensForDeposit(
      /*"0x499d11E0b6eAC7c0593d8Fb292DCBbF815Fb29Ae"*/ "0x7d1afa7b718fb893db30a3abc0cfc608aacfebb0",
      job.attributes.amount,
      { from: _userAddress }
    );
    //Deposit ERC20
    let txHash = await maticPlasma.depositERC20ForUser(
      /*"0x499d11E0b6eAC7c0593d8Fb292DCBbF815Fb29Ae"*/ "0x7d1afa7b718fb893db30a3abc0cfc608aacfebb0",
      _userAddress,
      job.attributes.amount,
      { from: _userAddress }
...
async function _checkSwapAmount(
  _jobId,
  _chain,
  _fromTokenAddress,
  _toTokenAddress,
  ...
  if (_fromTokenAddress == "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee") {
    try {
      ...
      let balanceResponse = await getBalancesByAddress(
        _fromTokenAddress,
        _chain
      );
      ...
      let txResponse = await fetch(
        `https://api.1inch.exchange/v3.0/${
          chain[_chain]
        }/swap?fromTokenAddress=${_fromTokenAddress}&toTokenAddress=${_toTokenAddress}&amount=${(
          parseInt(_amount) * 0.75
        ).toString()}&fromAddress=${_userAddress}&slippage=${_slippage}`
      );
...
async function doSwap(_jobId, _step) {
  ...
  window.ERC20TokencontractInstance = new web3.eth.Contract(
    erc20ABI,
    job.attributes.fromTokenAddress
  );
  ...
  if (
    ...
    const approve = await ERC20TokencontractInstance.methods
      .approve(
  ...
  if (_step == 0) {
    ...
    response = await fetch(
      `https://api.1inch.exchange/v3.0/${
        chain[job.attributes.toChain]
      }/swap?fromTokenAddress=${
        job.attributes.fromTokenAddress
      }&toTokenAddress=${_toTokenAddress}&amount=${
        job.attributes.amount
      }&fromAddress=${_userAddress}&slippage=${job.attributes.slippage}`
    );
  }
  if (_step == 1 || _step == 2) {
    await _checkSwapAmount(
    ...
    response = await fetch(
      `https://api.1inch.exchange/v3.0/${
        chain[job.attributes.fromChain]
      }/swap?fromTokenAddress=${
        job.attributes.fromTokenAddress
      }&toTokenAddress=${_toTokenAddress}&amount=${
        job.attributes.amount
...
async function getMyBalances() {
  try {
    ...
    let priceResponse = await Promise.all([
      ...
      axios.get(
        `https://api.coingecko.com/api/v3/simple/token_price/ethereum?contract_addresses=${response[2]}&vs_currencies=usd`
      ),
      axios.get(
        `https://api.coingecko.com/api/v3/simple/token_price/polygon-pos?contract_addresses=${response[3]}&vs_currencies=usd`
      ),
    ...
    const tokenBalances = response[1].map(function (value) {
      let imageUrl;
      if (priceResponse[4].data.tokens[value.tokenAddress])
        imageUrl = priceResponse[4].data.tokens[value.tokenAddress]["logoURI"];
      else if (priceResponse[5].data.tokens[value.tokenAddress])
        imageUrl = priceResponse[5].data.tokens[value.tokenAddress]["logoURI"];
      ...
      return {
        ...
        image: imageUrl,
        chain: value.chain,
        price: ethTokenPrices[value.tokenAddress]
          ? ethTokenPrices[value.tokenAddress].usd
          : polygonTokenPrices[value.tokenAddress].usd,
      };
    });
  ...
  try {
    ...
    for (var i = 0; i < responseTransactions.length; i++) {
      if (
        ...
        window.web3.eth.abi.encodeFunctionSignature("depositEtherFor(address)")
      ) {
...
async function getBalancesByAddress(_tokenAddress, _chain) {
  try {
    ...
    if (_chain == 0 && _tokenAddress == "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee") {
      ...
      balance = new BN(token) / Math.pow(10, 18);
      amount = token;
      ...
      balance = new BN(token.attributes.balance) / Math.pow(10, parseInt(token.attributes.decimals));
